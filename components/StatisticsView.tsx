    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-3 bg-white p-4 rounded-xl shadow-sm">
        ...
        <div className="flex-1">
           <h2 className="text-xl font-bold text-gray-800">Trends & Analytics</h2>
           <p className="text-xs text-gray-500">Track your progress over time</p>
        </div>
        ...
        <div className="flex bg-gray-100 p-1 rounded-lg">
           {(['7', '30', '90'] as TimeRange[]).map(r => (
             <button
               key={r}
               onClick={() => setRange(r)}
               className={`px-3 py-1 text-xs font-bold rounded-md transition ${range === r ? 'bg-white text-emerald-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
             >
               {r} Days
             </button>
           ))}
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
         <div className="bg-white p-4 rounded-xl shadow-sm border border-emerald-50">
            ...
            <p className="text-2xl font-bold text-emerald-600">{averageCals}</p>
            <p className="text-xs text-gray-400">Target: {user.targetCalories}</p>
         </div>
         <div className="bg-white p-4 rounded-xl shadow-sm border border-blue-50">
            <p className="text-xs text-gray-500 uppercase font-semibold">Latest Weight</p>
            <p className="text-2xl font-bold text-blue-600">{recordedWeights.length > 0 ? recordedWeights[recordedWeights.length - 1] : user.weight} <span className="text-sm">kg</span></p>
         </div>
         <div className="bg-white p-4 rounded-xl shadow-sm border border-purple-50">
            <p className="text-xs text-gray-500 uppercase font-semibold">Weight Change</p>
            <p className={`text-2xl font-bold ${recordedWeights.length > 1 ? (recordedWeights[recordedWeights.length - 1] - recordedWeights[0] < 0 ? 'text-green-500' : 'text-red-500') : 'text-gray-600'}`}>
         ...
         <div className="bg-white p-4 rounded-xl shadow-sm border border-orange-50">
            <p className="text-xs text-gray-500 uppercase font-semibold">Log Streak</p>
