    ...
    <div className="max-w-4xl mx-auto space-y-4">
      ...
      <div className="flex items-center gap-3 bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm transition-colors">
        ...
        <div className="flex-1">
           <h2 className="text-lg font-bold text-gray-800 dark:text-white">{formatDateReadable(date)}</h2>
      ...
      {activeTab === 'nutrition' ? (
        <div className="space-y-4 animate-fade-in">
             ...
             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md col-span-1 md:col-span-2 flex flex-col justify-center transition-colors">
                    <div className="flex justify-between items-end mb-2">
                        <div>
                            <span className="text-3xl font-bold text-gray-800 dark:text-white">{Math.round(totalMacros.calories)}</span>
                            <span className="text-gray-500 dark:text-gray-400 ml-1">/ {calorieGoal} kcal</span>
                        </div>
                    ...
                    <div className="mt-6 grid grid-cols-3 gap-4 text-center">
                        <div className="bg-blue-50 dark:bg-blue-900/30 p-2 rounded-lg">
                            <p className="text-xs text-blue-500 dark:text-blue-300 font-semibold uppercase">Carbs</p>
                            <p className="font-bold text-gray-700 dark:text-gray-200">{totalMacros.carbs.toFixed(2)}g</p>
                        </div>
                        <div className="bg-emerald-50 dark:bg-emerald-900/30 p-2 rounded-lg">
                            <p className="text-xs text-emerald-500 dark:text-emerald-300 font-semibold uppercase">Protein</p>
                            <p className="font-bold text-gray-700 dark:text-gray-200">{totalMacros.protein.toFixed(2)}g</p>
                        </div>
                        <div className="bg-red-50 dark:bg-red-900/30 p-2 rounded-lg">
                            <p className="text-xs text-red-500 dark:text-red-300 font-semibold uppercase">Fat</p>
                            <p className="font-bold text-gray-700 dark:text-gray-200">{totalMacros.fat.toFixed(2)}g</p>
                        </div>
                    </div>
                </div>

                <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md flex flex-col items-center justify-center transition-colors">
                     ...
                     <div className="flex gap-2 text-xs mt-2 text-gray-600 dark:text-gray-300">
                  ...
                  <div className="divide-y divide-gray-100 dark:divide-gray-700">
                      {todaysLogs.length === 0 ? (
                          ...
                          todaysLogs.map(log => (
                              <div 
                                key={log.id} 
                                className="p-4 hover:bg-gray-50 dark:hover:bg-gray-700 flex justify-between items-center transition cursor-pointer group"
                                onClick={() => onEditLog(log)}
                              >
                                  <div className="flex items-center gap-4">
                                      {log.image ? (
                                        <img src={log.image} alt={log.name} className="w-12 h-12 rounded-lg object-cover border border-gray-200 dark:border-gray-600" />
                                      ) : (
                                      ...
                                      <div>
                                          ...
                                          <p className="text-xs text-gray-500 dark:text-gray-400">{Math.round(log.calories)} kcal • P: {log.protein.toFixed(2)}g</p>
                                      </div>
                                  </div>
                                  <button 
